<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ArgPackagingWIPMag.aspx.cs" Inherits="FGA_PLATFORM.business.production.ArgPackagingWIPMag" %>
			...
			<div class="col-md-3  "  style="position:absolute;width:320px; margin-left:-15px; ">
				...
				<div class=" filter-wrapper" style="height:100px; width:320px; position:fixed" > 
						...
						<div>
							...
							<button class="btn btn-primary  btn-add" onclick="SaveData()" style="background-color: white;color: #00b8ce; height:30px; width:70px;margin-left: 25px;position: relative;">Receive</button>
						</div>
						...
						<select id ="sloc" style="color: black; height:30px;width:100px; margin-bottom: 5px; margin-left:13px" >
							...
							<button class="btn  btn-default btn-sm btn-add" style="background-color: white;color: #00b8ce; height:30px; width:70px;margin-left: 25px;position: relative; margin-bottom: 5px" onclick ="RemoveData()">Sent</button>
						</div>
				...
				<div class="panel-body filter-wrapper" style="height:95px;position:fixed;width: 850px"> 
					<form class="form-inline" >
						<div class="form-group">
							...
							<input type="text" id="_ibn"  placeholder="input"  style="color: black; height:30px;width:100px; "/>	
						</div>
						<div class="form-group" >
							<label class="form-label"style="margin-left:-25px">PartNO</label>
							<input type="text" id="_ipn" placeholder="input"  style="color: black; height:30px;width:100px;margin-left:-25px"/>	
						</div>	
						<div class="form-group">
							<label class="form-label"style="margin-left:-25px">Location</label>
							<input type="text" id="_iloc" placeholder="input"  style="color: black; height:30px;width:100px;margin-left:-25px"/>
						</div>	
						<div class="form-group">	
							<label class="form-label"style="margin-left:-25px">Date(from)</label> 
							<div id="date-popup" class="input-group date"> 
								<input type="text" id="_fdtxt" data-format="D, dd MM yyyy"  style="color: black; height:30px;width:90px;margin-left:-25px"/>
						...
						<div class="form-group">	
							...
							<div id="date-popup1" class="input-group date">
								<input type="text" id="_tdtxt" data-format="D, dd MM yyyy"  style="color: black; height:30px;width:90px;margin-left:-25px"/> 
								<span class="input-group-addon"><i class="fa fa-calendar"></i></span> 
							</div>
						</div>
                        ...
                        <div class="form-group"style="position:relative; margin-left:670px; margin-top:-35px">	
							 <input type ="button"  class="btn btn-primary btn-sm" style="background:white;color:#00b8ce" onclick="$('#editable').tableExport({ type: 'excel', tableName: 'ARG_Packaging', escape: 'false' })" name="export" value="Export" />
						</div>
                         <div class="form-group"style="position:relative; margin-left:750px;margin-top:-155px ">	
							 <input type ="button"  class="btn btn-primary btn-sm" style="background:white;color:#00b8ce" onclick="delData()" name="del" value="Delete" />
						</div>
<!-- 还没设置的按钮 -->
                        <div class="form-group"style="position:relative; margin-left:750px;margin-top:-115px ">	
	...
	<script type="text/javascript">
	    ...
	    $(document).ready(function () {
	        ...
	        {
	            for (var j = 0; j < 19;j++){
	                loc = loc + '<option>'+"M"+(i+1)+"-"+(j+1)+'</option>';
	            }
            }

            for (var i = 0; i < 2; i++) {
                for (var j = 0; j < 19; j++) {
                    loc = loc + '<option>' + "L" + (i + 1) + "-" + (j + 1) + '</option>';
                }
            }

            //for (var i = 0; i < 10; i++) {
            //    for (var j = 0; j < 19; j++) {
            //        loc = loc + '<option>' + "J" + (i + 1) + "-" + (j + 1) + '</option>';
            //    }
            //}

            for (var i = 0; i < 8; i++) {
                for (var j = 0; j < 19; j++) {
                    loc = loc + '<option>' + "K" + (i + 1) + "-" + (j + 1) + '</option>';
                }
            }

            for (var i = 0; i < 2; i++) {
                for (var j = 0; j < 19; j++) {
                    loc = loc + '<option>' + "O" + (i + 1) + "-" + (j + 1) + '</option>';
                }
            }
	    ...
	    function AddBarcodeNO(e)
	    {
	        ...
	        if (key == 13) {
	            ...
	            if (bpNO.indexOf(bp) <= 0) {
	                $.ajax({
	                    type: "Post",
	                    url: "ArgPackagingWIPMag.aspx/getBarcodeInfo",
	                    data: "{data:'" + bp + "'}",
	    ...
	    function queryData() {
	        ...
	        $.ajax({
	            type: "Post",
	            url: "ArgPackagingWIPMag.aspx/SearchData",
	    ...
	    function delData() {
	        ...
	        $.ajax({
	            type: "post",
	            url: "ArgPackagingWIPMag.aspx/deleteData",
	            data: "{data:'" + jsondata + "'}",
	            contentType: "application/json; charset=utf-8",
	            dataType: "json",
	            async: true,
	    ...
	    function SaveData() {
	        var sl = $('#sloc').val();
            if (sl != "All") {
                var r = confirm("Are you sure receive?");
                if (r == true)
                {
                    ...
                    $.ajax({
                        type: "post",
                        url: "ArgPackagingWIPMag.aspx/SaveScanRecord",
                        data: "{data:'" + jsondata + "',location:'" + sl + "'}",
                        contentType: "application/json; charset=utf-8",
                        dataType: "json",
                        async: false,
                        success: function (data) {
	    ...
	    function RemoveData() {
            ...
            if (r == true)
            {
                ...
                $.ajax({
                    type: "post",
                    url: "ArgPackagingWIPMag.aspx/outOfArea",
                    data: "{data:'" + jsondata + "'}",
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    async: false,
